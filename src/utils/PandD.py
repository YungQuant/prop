...
time_cnt = 0; hist_vals = []; profits = 0; bitchCunt = 0.15; atl = 9999; ath = 0; stop = 0;
while(1):
    try:
        crypto = 'BTC-GRC'
        tick = b.get_ticker(crypto)
        bal = b.get_balance(crypto[-3:])
        curr_price = float(tick['result']['Last'])
        if curr_price < atl:
            atl = curr_price

        if curr_price > ath:
            ath = curr_price
            stop = ath * (1 - bitchCunt)

        if curr_price > atl * 1.1:
            bitchCunt = 0.1
        if curr_price > atl * 1.25:
            bitchCunt = 0.07
        if curr_price > atl * 1.5:
            bitchCunt = 0.05

        if curr_price < stop:
            liquidate(crypto[-3:], 'bid')
        print("price:", tick['result'])
        print("ATL:", atl, "ATH:", ath, "Stop:", stop, "bitchCunt:", bitchCunt)
        print("balance:", bal['result'])
        print("\n")
