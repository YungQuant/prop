...
def write_that_shit(log, tick, kin, din, fin, perc, cuml, bitchCunt, bah):
...
def fucking_paul(tik, log, Kin, Din, Fin, save_max, max_len, bitchCunt, tradeCost):
    ...
    for i, closeData in enumerate(stock):
        ...
        if i > Kin:
            ...
            if ((roc > Din) and (stockBought == False and stopLoss == False)):
                ...
                bull += 1
                stockBought = True
            elif stockBought == True and (closeData > (buy[-1] * (1 + Fin))):
    ...
    if cuml > 1 + ((stock[-1] - stock[0]) / stock[0]) and len(perc) <= max_len and len(perc) > 1:
        write_that_shit(log, tik, Kin, Din, Fin, perc, cuml, bitchCunt, 1 + ((stock[-1] - stock[0]) / stock[0]))
        print(tik)
        print("len:", len(perc), "cuml:", cuml)
        print("BAH:", 1 + (stock[-1] - stock[0]) / stock[0])
        print("bitchCunt:", bitchCunt)
        print("Kin:", Kin, "Din:", Din, "Fin:", Fin)
        print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
...
for i, tick in enumerate(ticker):
    ...
    fileOutput.append("../../output/" + tick + "_cip3.1_cluster_compData_shortStop_7,17,17_output.txt")
        ...
        while (i < (k / 100) + 0.01):
            while (j < j2):
                while (d < d2):
                    ...
                    if i > 0:
                        if d == d1:
                            print("RATE-OF-CHANGE -or- CONSISTENT UNILATERAL PRESSURE MODEL -- short_stop edition")
                            print(i, "/", k / 100, int(np.floor(k)), "/", k2, j, "/", j2)
                        pillowcaseAssassination(fileTicker, k, i, d, fileOutput, save_max=1.01, max_len=3000000, bitchCunt=j, tradeCost=0.0025)
                    #     s += 0.002
                    # s = s1
                    d += 0.005
                d = d1
