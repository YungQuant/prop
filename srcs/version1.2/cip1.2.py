...
def BBmomma(arr, Kin, Din, Din1):
    lb, mb, ub = BBn(arr, Kin, Din, Din1)
    srange = ub - lb
    pos = arr[-1] - lb
    if srange > 0:
        return pos/srange
...
def fucking_paul(tik, log, Kin, Din, Kin1, Din1, save_max, max_len, bitchCunt, tradeCost):
    ...
    # WHO THE FUCK INTIALIZED CUML = 0.0 ??? THE STRATEGY STARTS WITH 1.0 (IE; 100% OF ITS INITIAL STARTING CAPITAL)
    ...
    for i, closeData in enumerate(stock):
        ...
        if i > int(max([Kin, Kin1, Din])):
                Kv = BBmomma(arr, int(np.floor(Kin)), Din1, Din1)
                kar.append(Kv)
            ...
            while (j < j2):
                while (d < d2):
                    ...
                    if i > 0:
                        ...
                        if pillowcaseAssassination(fileTicker, k, i, d, d, fileOutput, save_max=1.01, max_len=3000000, bitchCunt=j, tradeCost=0.0005) != 1:
