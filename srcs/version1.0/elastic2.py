...
def fucking_paul(stock, log, Kin, Din, save_max, max_len, bitchCunt, tradeCost):
    ...
    prices = [float(obj[-2]) for obj in stock]
    for i, closeData in enumerate(prices):
        #print(closeData)
        arr.append(closeData)
        if i > Kin:
            trainX, trainY = create_binary_ohlcv_dataset(stock[:len(arr)], Din)
            #print(trainX[0], trainY[0])
            model = linear_model.ElasticNet(alpha=29.91)
            model.fit(trainX, trainY)
            rarr = np.reshape(trainX[-1], [1, -1])
            #print(rarr)
...
for i, tick in enumerate(ticker):
    ...
    for k in range(len(orders)):
        ...
        for j in range(len(orders[k])):
            datum.append(orders[j][k])
        dataset.append(datum)

for i, tick in enumerate(ticker):
    #fileOutput.append(tick)
    fileOutput.append("../../output/elasticOhlcv_1h" + tick + "_10.20.18_output.txt")
#
