...
def createRangeDataset(data, lookback):
    lowX, lowY, highX, highY = [], [], [], []
    for i in range(len(data)):
        lowX.append(data[i:i + lookback])
        highX.append(data[i:i + lookback])
        lowY.append(min(data[i + lookback:i + (lookback * 2)]))
        highY.append(max(data[i + lookback:i + (lookback * 2)]))
...
def fucking_paul(tik, log, Kin, Din, save_max, max_len, bitchCunt, tradeCost):
    ...
    L, H = LinearRegression(), LinearRegression()

    lowX, lowY, highX, highY = createRangeDataset(stock[:int(np.floor(len(stock) * 0.8))], Kin)
    L.fit(lowX, lowY)
    H.fit(highX, highY)
...
ticker = ["BTC_ETH", "BTC_XMR", "BTC_DASH", "BTC_XRP", "BTC_MAID", "BTC_ZEC", "BTC_LTC"]
#ticker = ["BTC-LTC"]
...
for i, tick in enumerate(ticker):
    ...
    fileOutput.append("../../output/" + tick + "_cip2-mlsp4_7,6,17_output.txt")
