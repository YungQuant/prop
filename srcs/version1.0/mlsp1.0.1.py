...
def write_that_shit(log, tick, kin, thresh, perc, cuml, bitchCunt):
...
def fucking_paul(tick, Nin, thresh, log, save_max, max_len, bitchCunt, tradeCost):
    cuml, stock = [], []
    jj = 0
    while jj < len(tick) - 1:
        ...
        for i, closeData in enumerate(stock):
            ...
            if i > int(np.floor(len(stock) * .8)):
                ...
                if ((float(predict) > thresh) and (stockBought == False and stopLoss == False)):
                    buy.append(closeData * (1 + tradeCost))
                    bull += 1
                    stockBought = True
                if stockBought == True and closeData > maxP:
                    maxP = closeData
                elif ((float(predict) < 0 - thresh) and stockBought == True):
                    ...
                    stockBought = False
                    stopLoss = True
                elif ((float(predict) < 0 - thresh) and stopLoss == True):
                    stopLoss = False
        if stockBought == True:
        ...
        print("len:", len(perc), "cuml:", cuml[int(np.floor(jj / 3))])
        # plot(perc)
        # plot(cumld)
        if cuml[int(np.floor(jj / 3))] > save_max and len(perc) <= max_len:
            write_that_shit(log[int(np.floor(jj / 3))], tick[jj], Nin, thresh, perc, cuml[int(np.floor(jj / 3))], bitchCunt)
        jj += 3
...
for i, tick in enumerate(ticker):
    ...
    fileOutput.append("../../output/" + tick + "_mslp1.0.1_4.13.17_1dx0.8_1intervalPred_output.txt")
...
while t < 0.05:
    while j < 20:
        print(t, "/ 0.05", j, "/ 20")
        fucking_paul(fileTicker, int(np.floor(j)), t, fileOutput, save_max=1.00, max_len=100000, bitchCunt=0.50, tradeCost=0.0025)
        if j < 10:
            j += 1
        else:
            j *= 1.2
