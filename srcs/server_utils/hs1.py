...
cryptos = [currency for currency in ]
...
while(1):
    try:
        ...
        for i in range(len(pairs)):
            ...
            bid, ask, price = float(tick['Bid']), float(tick['Ask']), np.mean([float(tick['Ask']), float(tick['Bid'])])
            #print(tick['Bid'])
        ...
        print("horizontal scaling 1.0, credits: killabit, yungquant, buffman")
