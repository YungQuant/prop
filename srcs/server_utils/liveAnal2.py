...
def liveAnal2(currencies, interval):
    ...
    while 1:
        try:
            for i, currency in enumerate(currencies):
                quote = currency.split('/')[0]
                liveData = client.fetch_bid_asks(currency)
                timeStr = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f%Z")
                #print("liveData", liveData[currency]['bid'])
                bidPrice, askPrice, bidVol, askVol = liveData[currency]['bid'], liveData[currency]['ask'], liveData[currency]['bidVolume'], liveData[currency]['askVolume']
                ...
                liveDataArr[timeStr] = data
                print("liveDataArr:", liveDataArr)
...
liveAnal2(currencies, Tinterval)
