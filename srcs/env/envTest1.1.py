...
def global_warming(ticker, cuml=1, tradeCost=0.0025, rebal_tol=0.1, plt_bool=False):
    ...
    for y in range(len(fileTicker)):
        ...
        for i, stocks in enumerate(stock1[-90:]):
    ...
    for n in range(min([int(np.floor(len(cumulative_diffs[f]) * 1)) for f in range(len(cumulative_diffs))])):
        ...
        cuml = sum(allocs)
        if max(allocs) - min(allocs) > rebal_tol:
            rebalsss += 1
...
while k < k2:
    ...
    if rebalsss > 1 and len(results) > 2 and results[-1] > np.mean(results[:-1]):
        write_that_shit("../../output/envTest1.1_noMult_[-90:]_output_6,23,17.txt", k, rebalsss, results[-1], mdd)
