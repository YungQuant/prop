...
def get_the_shit(prices, alloc, lookback, n, len_buys):
    ...
    model = Sequential()
    model.add(Dense(lookback, input_shape=(1, lookback)))
    model.add(LSTM(lookback, activation='relu'))
    model.add(Dropout(0.15))
    model.add(Dense(lookback, activation='relu'))
    model.compile(loss='mean_squared_error', optimizer='Adam', metrics=['accuracy'])
    model.fit(X, Y, nb_epoch=20, batch_size=5, verbose=0)

    # R = LinearRegression(fit_intercept=True, normalize=True, n_jobs=8)
    # R.fit(X, Y)
    predict = model.predict(prices[-lookback:])
