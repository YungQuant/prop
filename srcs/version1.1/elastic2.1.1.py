...
def create_binary_h2d_ohlcv_dataset(dataset, look_back, ohlcv=-1, day=1.00001):
    look_back *= 24
    dataX, dataY = [], []
    for i in range(len(dataset)-1):
        datum = []
        if i > look_back:
            ...
            dataX.append(datum)
            # if dataset[i+1][-1] > dataset[i][-1]:
            if np.mean([d[ohlcv] for d in dataset[i+1:i+int(np.floor(day*24))]]) > dataset[i][ohlcv]:
                dataY.append(1)
            else:
                dataY.append(0)
    return np.array(dataX), np.array(dataY)
...
def decompose(arrOarrs):
    oarr = []
    for i in range(len(arrOarrs)):
        for k in range(len(arrOarrs[i])):
            oarr.append(float(arrOarrs[i][k]))
    return oarr
...
def write_that_shit(log, kin, din,  perc, cuml, mdd, bitchCunt):
    ...
    file.write("\nMDD:\t")
    file.write(str(mdd))
    file.write("\n [elastic, static bitchcunt distance] Cumulative Diff:\t")
    file.write(str(cuml))
    file.write("\n\n")
    file.close()
...
def fucking_paul(stock, log, Kin, Din, save_max, max_len, bitchCunt, tradeCost):
    ...
    prices = [float(obj[-1]) for obj in stock]

    trainX, trainY = create_binary_h2d_ohlcv_dataset(stock[:int(np.floor(Kin))], Din, day=0.5)
    testX, testY = create_binary_h2d_ohlcv_dataset(stock[int(np.floor(Kin)):], Din, day=0.5)
    print(f"train, test {np.array(trainX).shape} {np.array(trainY).shape} , {np.array(testX).shape} {np.array(testX).shape}")
    model = linear_model.ElasticNet(alpha=29.91)
    model.fit(trainX, trainY)

    for i, closeData in enumerate(prices):
        #print(closeData)
        arr.append(closeData)
        if i > Kin + Din:
            # trainX, trainY = create_binary_ohlcv_dataset(stock[int(np.floor(Kin)):i], Din)
            # print(stock[int(np.floor(Kin)):i])
            # print(trainX[0], trainY[0])
            # print(stock[i-Din:i])
            rarr = decompose(testX[i-Din-int(np.floor(Kin)):i-int(np.floor(Kin))])
            # rarr = decompose(stock[i-Din:i])
            rarr = np.reshape(rarr, [1, -1])
            # print(rarr)
            p = model.predict(rarr)
            #print(p, closeData)
            if ((p >= 0.5) and position == -1 or position == 0):
                buy.append(closeData * (1+tradeCost))
                bull += 1
                position = 1
            elif (p < 0.5) and position == 1 or position == 0:
...
for i, tick in enumerate(ticker):
    ...
    #     datum = []
    #     for j in range(len(orders[k])):
    #         datum.append(orders[k][j])
    #     dataset.append(datum)
    dataset.append(orders)

for i, tick in enumerate(ticker):
    #fileOutput.append(tick)
    fileOutput.append("../../output/elastic2.1MEXh2dOhlcvDe-Fucked_1h" + tick + "_12.9.18_output.txt")
#
...
def run():
    k1 = len(dataset[0]) * 0.099
    k2 = len(dataset[0]) * 0.101
    l1 = 2; l2 = 30
    # d1 = 2; d2 = 300
    # s1 = 2; s2 = 30
    j1 = 0.001; j2 = 0.8
        ...
        while (i < l2):
            while (j < j2):
                #while (d < d2):
                    #while (s < s2):
                if i > 0:
                    if i % 2 == 0: print(i, "/", l2, int(np.floor(k)), "/", k2, j, "/", j2)
