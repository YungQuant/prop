...
def create_orderbook_training_set(buy_arr, sell_arr, lookback):
    ...
    x, y = [], []
    k = 0
    while k < (len(buy_arr) - lookback - 20):
        x.append(sell_arr[k:k + lookback] + buy_arr[k:k + lookback])
        y.append(np.mean([float(sell_arr[k + lookback + 20]), float(buy_arr[k + lookback + 20])]))
        k += 2
    return np.array(x), np.array(y)
...
def write_that_shit(log, tick, Nin, predicts, diff, errorArry):
    ...
    file.write("\nmean percent error:\t")
    file.write(str(np.mean(errorArry)))
    if len(predicts) > 10:
...
def fucking_peter(tick, Nin, log):
    cuml, stock = [], []
    jj = 0
    while jj < len(tick) - 1:
        ...
        buy = [];sell = []; diff = []; perc = []; desc = []; kar = []; dar = []; cumld = [];
        ...
        #print("training x[-2:], y[-2:]", trainX[-2:], trainY[-2:])
        ...
        for i, closeData in enumerate(stock):
            ...
            if i > int(np.floor(len(stock) * .8)):
                ...
                if i < len(stock) - 10:
                    ...
                    errorArry.append(difference)
                    # print("arry_diff:", difference)
                    # print("predict:", predict)
                    if stock[i + 10] > stock[i] and (predict > closeData) or \
                                            stock[i + 10] < stock[i] and (predict < closeData):
                        diff.append(1)
        ...
        if np.mean(errorArry) < 0.02 and errorCnt < 1000:
            write_that_shit(log[int(np.floor(jj / 3))], tick[jj], Nin, predictArray, diff, errorArry)
            print("tik:", tick[jj], "\n",
                  "Nin:", Nin)
            if errorCnt > 0: print("tengo", errorCnt, "problemas)")
            print("avg prediction:", np.mean(predictArray))
            print("mean inverse directional error (% correct):", np.mean(diff))
            print("mean percent error:", np.mean(errorArry))
...
for m in range(len(nins)):
        fucking_peter(fileTicker, nins[m], fileOutput)
        # try:
