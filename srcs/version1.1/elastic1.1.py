...
def fucking_paul(stock, log, Kin, Din, save_max, max_len, bitchCunt, tradeCost):
    ...
    trainX, trainY = create_dataset(stock[:int(np.floor(Kin))], Din)
    model.fit(trainX, trainY)
...
for i, tick in enumerate(ticker):
    ...
    fileOutput.append("../../output/elastic_1h_phat" + tick + "_10.21.18_output.txt")
#
...
def run():
    k1 = len(dataset[0]) * 0.7; k2 = len(dataset[0]) * 0.71
    l1 = 3; l2 = 30
    # d1 = 2; d2 = 300
    # s1 = 2; s2 = 30
    j1 = 0.01; j2 = 0.90
